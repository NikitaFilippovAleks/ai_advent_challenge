...
Формирует оптимизированный список сообщений для отправки в GigaChat:
- Последние N сообщений отправляются как есть
- Старые сообщения заменяются суммаризациями (каждые M сообщений)
- Суммаризации хранятся в БД и переиспользуются
"""
...
async def build_context(conversation_id: str) -> list[dict]:
    """Формирует оптимизированный список сообщений для отправки в GigaChat.
