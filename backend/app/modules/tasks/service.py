...
_CLASSIFY_SYSTEM = (
    ...
    "запросы на сбор информации, аудит, проверку, поиск файлов, "
    "создание заметок, работу с системной информацией, "
    "любые запросы которые можно выполнить через вызов инструментов/функций\n"
    "- task: ТОЛЬКО сложная многошаговая задача, требующая НАПИСАНИЯ КОДА и поэтапной реализации "
    "(создать приложение, спроектировать систему, провести рефакторинг). "
    "Если запрос можно выполнить через вызовы инструментов — это chat, НЕ task."
)
...
class TaskService:
    ...
    _MCP_KEYWORDS = [
        ...
        # Инструменты system-сервера
        "время", "дата", "диск", "процесс", "систем",
        # Инструменты notes-сервера
        "заметк", "замётк", "запиши", "сохрани заметку",
        # Общие паттерны использования инструментов
        "аудит", "проверь", "покажи статус", "статус git",
        "git-статус", "список веток", "найди todo", "найди fixme",
        "найди все", "поиск", "сохрани результат",
    ]
