...
- `backend/mcp_servers/filesystem_server.py` — MCP-сервер активной работы с файлами проекта (7 инструментов: `list_project_files`, `read_project_file`, `find_usages`, `analyze_file`, `check_rules`, `compute_diff`, `write_doc`). Чтение разрешено по всему `/repo` (кроме `.git`, `node_modules`, бинарников и файлов > 512 KB), запись — строго в `/repo/docs/`. Используется ассистентом для сценариев «найти все места, где используется X», «проверить соответствие правилам/инвариантам», «подготовить unified diff правок», «сгенерировать ADR/changelog/отчёт в docs/». Тесты: `backend/tests/test_filesystem_server.py` (16 кейсов, запуск `docker compose exec backend python -m unittest tests.test_filesystem_server -v` или одноразово `docker compose run --rm backend python -m unittest tests.test_filesystem_server -v`)
